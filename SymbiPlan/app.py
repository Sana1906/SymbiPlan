...
def get_ai_recommendation(df, selected_location):
    ...
    except: return "Signal data analysis error. Please check source headers."

def recommend_data_pack(pricing_df, budget, operator):
    try:
        # Standardizing column searches for your telecom sheet
        # Likely columns: Operator, Price, Data, Validity, Plan Name
        filtered = pricing_df[
            (pricing_df['Price'].astype(float) <= budget) & 
            (pricing_df['Operator'].str.contains(operator, case=False, na=False))
        ]
        if filtered.empty: return None
        # Best value = highest price within the budget
        return filtered.sort_values(by='Price', ascending=False).iloc[0]
    except: return "Error processing telecom plans."
...
# --- 5. SUB PAGES ---
    ...
    try:
        ...
        operator = st.selectbox("Your Operator", df_plans['Operator'].unique())
        
        if st.button("Recommend Best Plan"):
            best = recommend_data_pack(df_plans, budget, operator)
            if best is not None:
                st.success(f"### Recommended Plan: {best['Plan Name']}")
                st.write(f"**Price:** ₹{best['Price']} | **Data:** {best['Data']} | **Validity:** {best['Validity']}")
            else:
                st.warning("No plans found for this budget. Try increasing the amount.")
    except:
        st.error("Could not load telecom data. Ensure the spreadsheet columns are named 'Operator', 'Price', 'Data', 'Validity', and 'Plan Name'.")
